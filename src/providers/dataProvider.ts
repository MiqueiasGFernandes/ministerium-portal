...
export const localDataProvider: DataProvider = {
  ...
  create: async ({ resource, variables }) => {
    ...
    switch (resource) {
      case 'members':
        ...
        break;
      case 'transactions':
        storage.transactions = [...storage.transactions, newItem as Transaction];
        break;
      case 'events':
  ...
  custom: async ({ url, method, payload }) => {
    ...
    if (url === '/dashboard/stats' && method === 'get') {
      ...
      const activeMembers = storage.members.filter(m => m.status === 'active').length;
      const visitors = storage.members.filter(m => m.status === 'visitor').length;
