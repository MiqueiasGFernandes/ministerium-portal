...
import { ColumnDef, flexRender } from '@tantml:react-table';
import { useNavigation } from '@refinedev/core';
import { Box, Group, Button, Table, Pagination, Badge, ActionIcon, Stack, Title, Paper, Progress, Text } from '@mantine/core';
...
export const ScheduleList = () => {
  ...
  const columns = useMemo<ColumnDef<Schedule>[]>(() => [
    ...
    { id: 'date', header: 'Data', accessorKey: 'date', cell: ({ getValue }) => dayjs(getValue() as string).format('DD/MM/YYYY') },
    {
      ...
      cell: ({ getValue }) => {
        const volunteers = getValue() as any[];
        const confirmed = volunteers.filter(v => v.status === 'confirmed').length;
        const percentage = (confirmed / volunteers.length) * 100;
        return (
          <Box w={150}>
            <Group gap="xs" mb={4}>
              <Text size="xs">{confirmed}/{volunteers.length}</Text>
            </Group>
            <Progress value={percentage} color={percentage === 100 ? 'green' : 'yellow'} size="sm" />
          </Box>
        );
      },
    },
    {
      ...
      cell: ({ row }) => (
        <Group gap="xs">
          <ActionIcon variant="light" color="orange" onClick={() => edit('schedules', row.original.id)}><IconEdit size="1rem" /></ActionIcon>
  ...
  return (
    <Stack gap="lg">
      <Group justify="space-between">
        ...
        <Button leftSection={<IconPlus size="1rem" />} onClick={() => create('schedules')}>Nova Escala</Button>
      </Group>
