...
export const generateUsers = (count: number = 10): User[] => {
  const roles: UserRole[] = [UserRole.ADMIN, UserRole.LEADER, UserRole.VOLUNTEER];
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      customFields: {
        ...
        estadoCivil: faker.helpers.arrayElement(['Solteiro', 'Casado', 'Viúvo', 'Divorciado']),
...
export const generateTransactions = (count: number = 200): Transaction[] => {
  const types: TransactionType[] = [TransactionType.INCOME, TransactionType.EXPENSE];
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const category = type === TransactionType.INCOME
...
export const generateMinistries = (users: User[]): Ministry[] => {
  ...
  const leaders = users.filter(u => u.role === UserRole.LEADER || u.role === UserRole.ADMIN);
...
export const generateEvents = (
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      attendees,
      maxAttendees: faker.helpers.maybe(() => faker.number.int({ min: 50, max: 200 })),
      status: faker.helpers.arrayElement(statuses),
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const volunteers: ScheduleVolunteer[] = faker.helpers
      ...
      .map((member, idx) => ({
        ...
        respondedAt: faker.helpers.maybe(() => faker.date.recent().toISOString()),
      }));
